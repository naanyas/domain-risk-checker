...
def _entry_pairs(df):
    """Pair interleaved results [root, url, root, url, ...] into one
    (root_row, url_row) tuple per submitted entry (v8.4)."""
    rows = [df.loc[i] for i in df.index]
    pairs = []
    i = 0
    while i < len(rows):
        r = rows[i]
        scope = r.get('scan_scope', 'root') if hasattr(r, 'get') else 'root'
        nxt = rows[i + 1] if i + 1 < len(rows) else None
        nxt_scope = (nxt.get('scan_scope', '') if (nxt is not None and hasattr(nxt, 'get')) else '')
        if scope == 'root' and nxt is not None and nxt_scope == 'url':
            pairs.append((r, nxt)); i += 2
        else:
            pairs.append((r, r)); i += 1
    return pairs


def _render_domain_detail(domain_data):
    """Render the full per-scope detail block for one result row (v8.4)."""
...
def display_results(results: list):
    ...
    # Summary metrics — one entry = root + URL pair
    _pairs_m = _entry_pairs(df)
    _approved = sum(1 for r, u in _pairs_m
                    if r.get('recommendation') == 'APPROVE' and u.get('recommendation') == 'APPROVE')
    _denied = len(_pairs_m) - _approved
    _avg = (sum(max(int(r.get('risk_score', 0) or 0), int(u.get('risk_score', 0) or 0))
                for r, u in _pairs_m) / len(_pairs_m)) if _pairs_m else 0.0

    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("Entries Analyzed", len(_pairs_m))
    with col2:
        st.metric("✅ Both Approve", _approved)
    with col3:
        st.metric("🚫 Flagged (root or URL)", _denied)
    with col4:
        st.metric("Avg Risk (worst of 2)", f"{_avg:.1f}")
    
    # Tabs for different views
    tab1, tab2, tab3 = st.tabs(["📋 Summary View", "📊 Full Details", "⬇️ Download"])
    
    with tab1:
        # Summary table — ONE row per submitted entry, with a score+result
        # column for each scope (root vs submitted URL).  v8.4.
        ...
        # Pivot interleaved rows into one row per entry: score + result per scope.
        _pairs = _entry_pairs(df)
        _rows = []
        for _r, _u in _pairs:
            _lbl = str(_u.get('display_label', _r.get('domain', '')) or '').replace(' (page)', '')
            _rows.append({
                'Domain / URL': _lbl,
                'Root Score': int(_r.get('risk_score', 0) or 0),
                'Root': _r.get('recommendation', ''),
                'URL Score': int(_u.get('risk_score', 0) or 0),
                'URL': _u.get('recommendation', ''),
                'Threats': _build_threat_indicator(_u) or _build_threat_indicator(_r),
                'Summary': _u.get('summary', ''),
            })
        summary_df = pd.DataFrame(_rows, columns=[
            'Domain / URL', 'Root Score', 'Root', 'URL Score', 'URL', 'Threats', 'Summary'])
        ...
        styled_df = summary_df.style.map(
            color_recommendation, subset=['Root', 'URL']
        ).map(
            color_score, subset=['Root Score', 'URL Score']
        )

        column_config = {
            'Domain / URL': st.column_config.TextColumn('Domain / URL', width='large'),
            'Root Score': st.column_config.NumberColumn('Root', width='small',
                          help='Registrable-root risk score'),
            'Root': st.column_config.TextColumn('Root ✓', width='small'),
            'URL Score': st.column_config.NumberColumn('URL', width='small',
                         help='Submitted URL / subdomain risk score'),
    ...
    _sel = st.selectbox(
        ...
        list(range(len(_pairs))),
        format_func=lambda k: _pairs[k][1].get("display_label", _pairs[k][0].get("domain", "")),
    )
    if _sel is not None and _pairs:
        _root_row, _url_row = _pairs[_sel]
        st.markdown(f"### 🌐 Registrable root — `{_root_row.get('domain', '')}`")
        _render_domain_detail(_root_row)
        st.markdown('---')
        st.markdown(f"### 🔗 Submitted URL / subdomain — `{_url_row.get('display_label', '')}`")
        _render_domain_detail(_url_row)
